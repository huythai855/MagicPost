...
const Shipper = () => {
    ...
    return <> 
        ...
        <table>
            <thead>
                <tr>
                    <th> ID <FaSort onClick={() => sortingnum("id")}/> </th>
                    <th>Người gửi <FaSort onClick={() => sorting("sender")}/></th>
                    <th>Ngày gửi <FaSort onClick={() => sorting("date")}/></th>
                    <th>Điểm đến <FaSort onClick={() => sorting("reclo")}/></th>
                    <th>Loại sản phẩm <FaSort onClick={() => sorting("type")}/></th>
                    <th>Trạng thái <FaSort onClick={() => sorting("status")}/></th>
                    <th>Hành động</th>
                </tr>
            </thead>
            <tbody>
                    {currentItems.filter((curList) => {
                        return search.toLowerCase() === ''
                        ? curList
                        : curList.sender.toLowerCase().includes(search);
                    })
                    .map((curList) => {
                const { id, sender, date, reclo, type, status } = curList;
