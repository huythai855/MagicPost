export { default as DiemGiaoDich } from "./DiemGiaoDich";

export { default as DiemTapKet } from "./DiemTapKet";
export { default as Overview } from "./Overview";
export { default as MagicPost } from "./MagicPost";
export { default as DSNhanVien } from "./DSNhanVien";
export { default as DanhSachDonHang } from "./DanhSachDonHang";
export { default as DanhSachNgoaiKhu } from "./DanhSachNgoaiKhu";
export { default as DanhSachNgoaiKhuNew } from "./DanhSachNgoaiKhuNew";
export { default as DanhSachDonHangDenTKV } from "./DanhSachDonHangDenTKV";
export { default as DanhSachDonHangDiTKV } from "./DanhSachDonHangDiTKV";

export { default as FormTapKet } from "./FormTapKet";

export { default as DanhSachDonNoiKhu } from "./DanhSachDonNoiKhu";
export { default as DanhSachNK } from "./DanhSachNK";

export { default as FormBienNhan } from "./FormBienNhan";
export { default as ChiTietDonHang } from "./ChiTietDonHang";
export { default as ChiTietDonHangGDV } from "./ChiTietDonHangGDV";

export { default as Login } from "./Login";
export { default as Shipper } from "./Shipper";
